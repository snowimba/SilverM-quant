...
const fetchHistory = async () => {
  try {
    const response = await axios.get('/api/agent/history?limit=20')
...
const loadHistoryItem = (item: any) => {
  ...
  symbol.value = data.symbol || item.symbol || ''
  error.value = ''
}
...
<template>
  <div class="max-w-6xl mx-auto">
    ...
    <!-- History List -->
    <div v-if="historyList.length > 0" class="glass-card p-6 mt-6">
      <h3 class="text-lg font-semibold mb-4 text-slate-200">历史分析记录</h3>
      <div class="space-y-2 max-h-80 overflow-y-auto">
        <div
          v-for="(item, idx) in historyList" :key="idx"
          class="flex items-center justify-between p-3 rounded-lg bg-slate-800/50 hover:bg-slate-700/50 cursor-pointer transition-colors"
          @click="loadHistoryItem(item)"
        >
          <div class="flex items-center gap-3">
            <span class="text-white font-medium">{{ item.result?.symbol || item.symbol }}</span>
            <span class="text-slate-400 text-sm">{{ item.result?.trade_date || item.trade_date }}</span>
          </div>
          <div class="flex items-center gap-2">
            <span class="text-xs px-2 py-1 rounded" :class="item.result?.research?.recommendation === '买入' ? 'bg-green-500/20 text-green-400' : item.result?.research?.recommendation === '卖出' ? 'bg-red-500/20 text-red-400' : 'bg-slate-500/20 text-slate-400'">
              {{ item.result?.research?.recommendation || '观望' }}
            </span>
            <span class="text-slate-500 text-xs">{{ item.created_at?.split(' ')[1]?.slice(0,5) || '' }}</span>
          </div>
        </div>
      </div>
    </div>

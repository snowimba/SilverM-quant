...
const error = ref('')
const currentSource = ref<'tushare' | 'baostock'>('baostock')
...
const startScanSignals = async () => {
  ...
  try {
    const response = await axios.post('/api/data-update/scan-signals', {
      workers: workers.value
    })
    if (response.data.success) {
      currentTaskId.value = response.data.task_id
    ...
    error.value = e.response?.data?.error || '触发信号扫描失败'
    console.error(e)
  } finally {
        ...
        <div class="form-section">
          ...
          <div class="form-group">
            ...
            <div class="btn-group">
              ...
              <button class="btn btn-warning" @click="startScanSignals" :disabled="scanning">
                {{ scanning ? '扫描中...' : '📡 扫描信号' }}
              </button>
